...
@PreAuthorize(HasPerm('medical:bo:sync'))
@Log(title='血氧饱和度', business_type=BusinessType.UPDATE)
def sync_blood_oxygen():
    """同步血氧饱和度数据"""
    # 处理OPTIONS请求
    ...
    try:
        ...
        # 同步血氧饱和度表
        inserted, updated = synchronizer.synchronize_table('blood_oxygen_saturation')
