...
def create_table(cursor):
    """创建ppg表"""
    # 先删除表（如果存在）
    cursor.execute("DROP TABLE IF EXISTS ppg")
...
def import_data():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            # 读取CSV文件
            with open('sql/ppg.csv', 'r', encoding='utf-8-sig') as f:
                ...
                # 提交事务
                connection.commit()
                print("PPG数据导入完成，共导入{}条记录".format(len(batch)))
...
def insert_data(cursor, batch):
    ...
    INSERT INTO ppg (
        ...
        ppg_path, external_id, metadata_version
    ) VALUES (
        %s, %s, %s, %s, %s,
        %s, %s, %s
    )
