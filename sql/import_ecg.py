...
def import_data():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            # 读取CSV文件
            with open('sql/ecg.csv', 'r', encoding='utf-8-sig') as f:
                ...
                # 提交事务
                connection.commit()
                print("ECG数据导入完成，共导入{}条记录".format(len(batch)))
...
def insert_data(cursor, batch):
    ...
    INSERT INTO ecg (
        ...
        ecg_path, external_id, metadata_version
    ) VALUES (
        %s, %s, %s, %s, %s,
        %s, %s, %s
    )
